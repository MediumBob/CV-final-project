...
imds = imageDatastore([data_directory s 'training_faces' s '*.bmp']);
imgs = readall(imds);
...
imds = imageDatastore([data_directory s 'training_nonfaces' s '*.jpg']);
imgs = readall(imds);
...
% save results
pathname = fileparts('training_directory');
matfile = fullfile(pathname, 'trainingdataset.mat');

save matfile faces nonfaces face_integrals nonface_integrals
